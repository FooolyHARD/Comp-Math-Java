...
@Component
public class ExponentialMethod implements ApproximationMethod {

    private final LinearMethod linearMethod;

    @Autowired
    public ExponentialMethod(LinearMethod linearMethod) {
    ...
    public MethodResult solve(List<PointDto> pointDtos) {
        ...
        for (PointDto p : pointDtos) {
            ...
            double y = p.getY();
            if (y <= 0) {
                return new MethodResult(ApproximationMethodEnum.EXPONENTIAL,
                        Double.MAX_VALUE,
                        new ArrayList<>(),
                        new ArrayList<>(),
                        new ArrayList<>(),
        ...
        return linearMethod.solve(newPointDtos, pointDtos, true, ApproximationMethodEnum.EXPONENTIAL);
    }
}
